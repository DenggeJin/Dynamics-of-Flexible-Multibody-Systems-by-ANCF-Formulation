!***************************************************************************************************
!- PURPORSE:
!  本程序用于生成非线性方程的雅可比矩阵，并求解牛顿迭代过程中的线性方程组
!
!- CALL PROCEDURES:
!     JAC,用于生成非线性方程组的雅可比矩阵JACOBI
!     GET_NZNUM,用于得到雅可比矩阵中非零元素数目
!     CSR3,将雅可比矩阵采用一维变带宽存储到a,ia,ja
!
!- CALLED BY:
!     NEWTONSOLVER.F90
!
...
!-----INPUT:XX，当下的“系统广义位移+拉格朗日乘子向量”
!           b，由NLEQNS子程序生成的方程残差
!-----OUTPUT：x，方程自变量（此处即XX）下一次迭代的增量deta_XX
